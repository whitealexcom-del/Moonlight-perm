/* Moonlight PWA — service worker
   ...
   сети (VPN/iOS). Логика:
   - HTML (навигация): сначала сеть с таймаутом 12 сек → значит онлайн всегда свежая
   ...
   ВАЖНО: index.html весит ~3.9МБ (все картинки зашиты внутрь base64). Таймаут в 3 сек
   был рассчитан на обычную VPN-заглушку, но на медленном мобильном интернете скачать
   3.9МБ за 3 сек не успевает почти никогда — тогда открытие с экрана домой падало на
   кэш, а если кэша ещё не было (первый запуск / кэш не сохранился) — экран оставался
   пустым/чёрным и "обновить" не получалось. Подняли до 12 сек. */
var CACHE = "moonlight-v1-3-2";
...
self.addEventListener("fetch", function(e){
  ...
  if(isNav){
    e.respondWith(
      netWithTimeout(req, 12000).then(function(res){
        try {
          var copy = res.clone();
